...
function getData() { firebase.database().ref("/"+room_name).on('value', function(snapshot) { document.getElementById("output").innerHTML = ""; snapshot.forEach(function(childSnapshot) { childKey  = childSnapshot.key; childData = childSnapshot.val(); if(childKey != "purpose") {
    ...
    name_with_tag = "<h4 id='nmh'> "+ name +"<img class='user_tick' src='tick.png' width='50'></h4>";
    message_with_tag = "<h4 class='message_h4' id='msgh'>" + message + "</h4>";
...
function updateLike(message_id){
    console.log("new likes for " + message_id);
    button_id = message_id;
    likes = document.getElementById(button_id).value;
    updated_likes = Number(likes) + 1;
    console.log("new amount" + updated_likes);
